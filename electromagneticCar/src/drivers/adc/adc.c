...
void GetAdc()
{
  ADresult[0] = ad_ave(ADC0,SE8,ADC_8bit,20);  //  PTB0   左平行  
  ADresult[1] = ad_ave(ADC0,SE9,ADC_8bit,20);   // PTB1   右内
  ADresult[2] = ad_ave(ADC0,SE12,ADC_8bit,20);  //PTB2    右 平行
  ADresult[3] = ad_ave(ADC0,SE13,ADC_8bit,20);   //PTB3    左内
  ADresult[4] = ad_ave(ADC0,SE10,ADC_8bit,20);   //PTA7    左外
  ADresult[5] = ad_ave(ADC0,SE11,ADC_8bit,20);   //PTB8     右外
  for(int i = 0; i<6; i++)
  {
      D5110[6+i] = ADresult[i];
  }
